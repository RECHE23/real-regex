...
public:
   ...
   */
  constexpr compiler(const ast& tree, flags f)
    : tree_(tree),
      flags_(f)
  {}
...
private:
   ...
   */
  static constexpr void emit(dynamic_program& prog, instr in)
  {
    if (prog.code.size() >= max_program_size) {
      throw regex_error("program too large", 0);
    }
   ...
   */
  static constexpr void patch_x(dynamic_program& prog, std::int32_t pc, std::int32_t target)
  {
   ...
   * \param[in]     target Instruction index to branch to.
   */
  static constexpr void patch_y(dynamic_program& prog, std::int32_t pc, std::int32_t target)
  {
   ...
   */
  constexpr void emit_repeat(dynamic_program& prog, const ast_node& node) const
  {
 ...
 * \throws real::regex_error if the program exceeds \ref max_program_size.
 */
constexpr dynamic_program compile(const ast& tree, flags f)
{
