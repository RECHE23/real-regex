...
#include "real/unicode/unicode_props.hpp" // word_ranges / space_ranges / is_space_cp
#include "real/unicode/utf8.hpp"          // decode_codepoint_strict — body stop-scan
...
namespace real::detail {
  ...
  /*!
   * \brief Whether \p cc is exactly text-mode `[^\s]` / `\S` (accepts cp iff !\ref is_space_cp).
   *
   * Used to arm the \ref pattern_hints::rare_disc_s_plus hint -- the dedicated body stop-scan ends at
   * whitespace via \ref is_space_cp, so the class must not be a stricter/looser set.
   *
   * **Conservative correctness (D1c):** ASCII is checked exhaustively; non-ASCII is an
   * **exact bidirectional range equality** against the complement of \ref space_ranges on
   * `[U+0080, U+10FFFF]` (linear merge of two sorted lists). No sampling — a near-`\S`
   * class (e.g. `[^\s\u0500]`) must **decline**.
   */
  constexpr bool is_exactly_not_space_cp_class(const cp_class&             cc,
                                               std::span<const code_range> cp_ranges)
  {
    // ASCII: accept b iff !is_space_cp(b) — exhaustive over 0..0x7F.
    for (int b = 0; b < 0x80; ++b) {
      const bool want {!is_space_cp(static_cast<char32_t>(b))};
      if (cc.ascii.test(static_cast<std::uint8_t>(b)) != want) {
        return false;
      }
    }
    // Expected high ranges: complement of space_ranges over [0x80, 0x10FFFF].
    // space_ranges is sorted non-overlapping; walk gaps between multi-byte space runs.
    constexpr char32_t hi_lo                            {0x80U};
    constexpr char32_t hi_hi                            {0x10FFFFU};
    // Materialized expected ranges (at most space_ranges_size + 1 gaps).
    code_range         expected[space_ranges_size + 1U] {};
    std::size_t        n_exp                            {0};
    char32_t           cursor                           {hi_lo};
    for (const code_range& r : space_ranges) {
      if (r.hi < hi_lo) {
        continue;
      }
      if (r.lo > hi_hi) {
      ...
      const char32_t s_lo {r.lo < hi_lo ? hi_lo : r.lo};
      const char32_t s_hi {r.hi > hi_hi ? hi_hi : r.hi};
      if (cursor < s_lo) {
        expected[n_exp++] = {.lo = cursor, .hi = s_lo - 1U};
      }
      cursor = s_hi + 1U;
      if (cursor > hi_hi) {
        break;
    ...
    if (cursor <= hi_hi) {
      expected[n_exp++] = {.lo = cursor, .hi = hi_hi};
    }
    // Class high ranges must equal `expected` exactly (same count, same bounds).
    if (static_cast<std::size_t>(cc.range_count) != n_exp) {
      return false;
    }
    for (std::uint32_t k {0}; k < cc.range_count; ++k) {
      const std::size_t idx {static_cast<std::size_t>(cc.range_begin) + k};
      if (idx >= cp_ranges.size()) {
        return false;
      }
      const code_range& got {cp_ranges[idx]};
      if (got.lo != expected[k].lo || got.hi != expected[k].hi) {
        return false;
      }
    }
    return true;
  ...
  /*!
   * \brief Arms \ref pattern_hints::rare_disc_s_plus when the program is capture-free
   *        rare-disc scheme + trailing text-mode `[^\s]+` / `\S+`.
   *
   * Shape key is the **compiled form** (not a hard-coded `"http"` string): rare_disc fields
   * already describe the scheme; the suffix must be a greedy not-space `klass_cp` loop and
   * the only saves are group 0 (slot_count shape: save 0 … save 1, match). Capturing patterns
   * decline — the dedicated route only fills group 0.
   */
  constexpr void extract_rare_disc_s_plus(std::span<const instr>      code,
                                          std::span<const cp_class>   cp_classes,
                                          std::span<const code_range> cp_ranges,
                                          pattern_hints&              hints)
  {
    hints.rare_disc_s_plus = false;
    if (hints.rare_disc < 0 || hints.rare_disc_after_len == 0) {
      return;
    }
    // Structural: optional mono-byte must not equal the disc (greedy re-walk would
    // consume the disc as the optional and miss a scheme the VM still matches).
    if (hints.rare_disc_opt >= 0 && hints.rare_disc_opt == hints.rare_disc) {
      return;
    }
    // Capture-free: only save 0 and save 1 (group 0). Any other save → decline.
    for (const instr& in : code) {
      if (in.op == opcode::save && in.arg16 > 1) {
        return;
      }
    }
    // Walk: save 0, then the same fixed/opt/disc/after byte run extract_rare_discriminant saw,
    // then klass_cp + 3 cont + split-loop + save 1 + match.
    std::size_t pc {0};
    if (pc >= code.size() || code[pc].op != opcode::save || code[pc].arg16 != 0) {
      return;
    }
    ++pc;
    ...
    if (hints.rare_disc_opt >= 0) {
      if (pc >= code.size() || code[pc].op != opcode::split) {
        return;
      }
      const std::int32_t pri {code[pc].primary_target};
      const std::int32_t sec {code[pc].secondary_target};
      if (pri != static_cast<std::int32_t>(pc + 1) || sec != static_cast<std::int32_t>(pc + 2)
          || pc + 1 >= code.size() || code[pc + 1].op != opcode::byte
          || static_cast<std::int16_t>(static_cast<std::uint8_t>(code[pc + 1].arg8))
          != hints.rare_disc_opt) {
        return;
      }
      pc = static_cast<std::size_t>(sec);
    ...
    if (pc >= code.size() || code[pc].op != opcode::byte
        || static_cast<char>(code[pc].arg8) != static_cast<char>(hints.rare_disc)) {
      return;
    }
    ++pc;
    for (std::uint8_t j {0}; j < hints.rare_disc_after_len; ++j) {
      if (pc >= code.size() || code[pc].op != opcode::byte
          || static_cast<char>(code[pc].arg8) != hints.rare_disc_after[j]) {
        return;
      }
      ++pc;
    }
    // Trailing greedy klass_cp+ (not-space).
    if (pc + 5 >= code.size() || code[pc].op != opcode::klass_cp || code[pc + 1].op != opcode::klass
        || code[pc + 2].op != opcode::klass || code[pc + 3].op != opcode::klass
        || code[pc + 4].op != opcode::split) {
      return;
    }
    const std::int32_t cp_idx {code[pc].arg16};
    if (cp_idx < 0 || static_cast<std::size_t>(cp_idx) >= cp_classes.size()) {
      return;
    }
    if (code[pc + 4].primary_target != static_cast<std::int32_t>(pc)
        || code[pc + 4].secondary_target != static_cast<std::int32_t>(pc + 5)) {
      return;
    }
    std::size_t q {pc + 5};
    if (q >= code.size() || code[q].op != opcode::save || code[q].arg16 != 1) {
      return;
    }
    ++q;
    if (q >= code.size() || code[q].op != opcode::match || q + 1 != code.size()) {
      return;
    }
    if (!is_exactly_not_space_cp_class(cp_classes[static_cast<std::size_t>(cp_idx)], cp_ranges)) {
      return;
    }
    hints.rare_disc_s_plus = true;
  }
                                          ...
                                          std::span<const lookaround_sub> lookarounds = {})
  {
    ...
    // Capture-free rare_disc + trailing [^\s]+ / \S+: one dedicated confirm route (D1b).
    extract_rare_disc_s_plus(code, cp_classes, cp_ranges, hints);
    return hints;
  }

  /*!
   * \brief ASCII whitespace bitmasks derived once from \ref space_ranges (not a hand list).
   *
   * Bit \c b is set iff \c is_space_cp(b) for \c b < 0x80. `inline constexpr` values so every
   * optimisation level const-folds them (D1c-land polish). Used by \ref not_space_run_end.
   */
  namespace ascii_space_bits_detail {
    constexpr std::uint64_t make_lo() // bits 0..63
    {
      std::uint64_t m {0};
      for (const code_range& r : space_ranges) {
        for (char32_t cp {r.lo}; cp <= r.hi && cp < 64U; ++cp) {
          m |= (std::uint64_t {1} << cp);
        }
      }
      return m;
    ...
    constexpr std::uint64_t make_hi() // bits 64..127
    {
      std::uint64_t m {0};
      for (const code_range& r : space_ranges) {
        for (char32_t cp {r.lo < 64U ? 64U : r.lo}; cp <= r.hi && cp < 0x80U; ++cp) {
          m |= (std::uint64_t {1} << (cp - 64U));
        }
      }
      return m;
    }
  ...
  inline constexpr std::uint64_t ascii_space_bitmask_lo {ascii_space_bits_detail::make_lo()};
  inline constexpr std::uint64_t ascii_space_bitmask_hi {ascii_space_bits_detail::make_hi()};

  //! \brief Whether ASCII byte \p b is text-mode `\s` (bitmask from \ref space_ranges).
  constexpr bool is_ascii_space_byte(std::uint8_t b) noexcept
  {
    if (b >= 0x80U) {
      return false;
    }
    if (b < 64U) {
      return (ascii_space_bitmask_lo & (std::uint64_t {1} << b)) != 0U;
    }
    return (ascii_space_bitmask_hi & (std::uint64_t {1} << (b - 64U))) != 0U;
  }

  /*!
   * \brief End index of a greedy text-mode `[^\s]+` / `\S+` run starting at \p pos.
   *
   * Returns \p pos if the run is empty (space / invalid / EOS at \p pos). Otherwise the
   * first whitespace code point or the first malformed UTF-8 lead, or `text.size()`.
   * ASCII uses a bitmask **derived from** \ref space_ranges; multi-byte uses decode +
   * \ref is_space_cp (same table). No hand-coded lead-byte list.
   */
  constexpr std::size_t not_space_run_end(std::string_view text,
                                          std::size_t      pos)
  {
    if (pos >= text.size()) {
      return pos;
    }
    const auto b0 {static_cast<std::uint8_t>(text[pos])};
    if (b0 < 0x80U) {
      if (is_ascii_space_byte(b0)) {
        return pos;
      }
    }
    else {
      const decoded_codepoint dc {decode_codepoint_strict(text, pos)};
      if (!dc.valid || is_space_cp(static_cast<char32_t>(dc.cp))) {
        return pos;
      }
    }
    std::size_t i {pos};
    while (i < text.size()) {
      ...
      if (b < 0x80U) {
        if (is_ascii_space_byte(b)) {
          return i;
        }
        ++i;
        // Unroll short ASCII non-space runs (typical URL bodies).
        while (i + 4U <= text.size()) {
          const auto b1 {static_cast<std::uint8_t>(text[i])};
          const auto b2 {static_cast<std::uint8_t>(text[i + 1U])};
          const auto b3 {static_cast<std::uint8_t>(text[i + 2U])};
          const auto b4 {static_cast<std::uint8_t>(text[i + 3U])};
          if (b1 >= 0x80U || b2 >= 0x80U || b3 >= 0x80U || b4 >= 0x80U) {
            break;
          }
          if (is_ascii_space_byte(b1)) {
            return i;
          }
          if (is_ascii_space_byte(b2)) {
            return i + 1U;
          }
          if (is_ascii_space_byte(b3)) {
            return i + 2U;
          }
          if (is_ascii_space_byte(b4)) {
            return i + 3U;
          }
          i += 4U;
        }
