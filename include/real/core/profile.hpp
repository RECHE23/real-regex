 ...
 * functions — no branch, no thread_local touch, no object-layout change. Counters live
 * only in a \c thread_local struct under \c real::detail::prof (never as members of
 * engine objects). Enable with \c -DREAL_PROFILE on a dedicated measurement binary.
 *
...
namespace real::detail::prof {
  ...
  //! \brief Named once-per-dispatch routes (Tier 1).
  ...
  //! \brief Thread-local counters — never stored on engine objects.
  struct counters
  {
