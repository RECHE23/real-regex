...
TEST(posix_ere_untranslatable_returns_nullopt)
{
  ...
  EXPECT(!d::translate_ere("(|a)*").has_value()); // empty alternation branch: std rejects it in POSIX
  EXPECT(!d::translate_ere("(a|)*").has_value()); // empty-last branch, likewise
  EXPECT(!d::translate_ere("ab|").has_value());   // a trailing empty branch
  EXPECT(d::translate_ere("(a|b)").has_value());  // a non-empty alternation still translates
