...
// The lead-byte table in utf8.hpp replaced three mask tests, and those tests are kept HERE as the
// oracle rather than deleted with the code: the table encodes a length for every byte including the
// ones that are always malformed (0xC0/0xC1 overlong-only, 0xF5-0xF7 past U+10FFFF), and the failure
// `length` is part of the decoder's contract -- callers report the bytes examined, and the
// malformed-UTF-8 matrix pins what they do with them. Give 0xC0 a length of 1 because "it can never
// be valid anyway" and every assertion in this file still passes while that contract quietly moves.
namespace {
  ...
  //! The decoder as it stood before the table, verbatim.
  constexpr mask_decoded decode_by_mask_tests(std::string_view text,
                                              std::size_t      pos)
  {
    const auto lead {static_cast<std::uint8_t>(text[pos])};
    if (lead < 0x80U) {
      return {.cp = lead, .length = 1, .valid = true};
    }
    std::size_t   length {};
    std::uint32_t cp     {};
    std::uint32_t min_cp {};
    ...
    for (std::size_t i = 1; i < length; ++i) {
      if (pos + i >= text.size()) {
        return {.cp = 0, .length = i, .valid = false};
      }
    ...
    if (cp < min_cp) {
      return {.cp = cp, .length = length, .valid = false};
    }
  ...
  bool agrees(const std::string& s)
  {
    const mask_decoded                    a {decode_by_mask_tests(s, 0)};
    const real::detail::decoded_codepoint b {real::detail::decode_codepoint_strict(s, 0)};
    return a.cp == b.cp && a.length == b.length && a.valid == b.valid;
  }
} // namespace

TEST(utf8_lead_table_reproduces_the_mask_tests)
{
  std::size_t checked  {0};
  std::size_t diverged {0};
  // Every 1- and 2-byte sequence exhaustively, then a third and fourth byte drawn from every class
  // the decoder distinguishes (ASCII, continuation at both ends of its range, and three non-leads).
  for (int x {0}; x < 256; ++x) {
    const auto bx {static_cast<char>(x)};
    checked  += 1;
    diverged += agrees(std::string(1, bx)) ? 0 : 1;
    for (int y {0}; y < 256; ++y) {
      const auto by {static_cast<char>(y)};
      checked  += 1;
      diverged += agrees(std::string {bx, by}) ? 0 : 1;
      for (const int z : {0x00, 0x41, 0x80, 0xBF, 0xC0, 0xF0, 0xFF}) {
        const auto bz {static_cast<char>(z)};
        checked  += 2;
        diverged += agrees(std::string {bx, by, bz}) ? 0 : 1;
        diverged += agrees(std::string {bx, by, bz, static_cast<char>(0x80)}) ? 0 : 1;
      }
    }
  }
  EXPECT(checked > 900000);
  EXPECT(diverged == 0);
}
