...
constant. For a threshold whose test cases sit far from its boundary, a 4x move can legitimately
change no outcome. Read a row as a question, not a verdict, and answer it by bracketing the constant
with cases just either side of it (see tests/engine/test_ac_density_gate.cpp for both shapes).
...
def main():
    ...
    for path, name, val, decl in consts:
        ...
        print(f"{name:<34}{val:>8}  {r1:>10}{r2:>10}   {'guarded' if guarded else 'UNGUARDED'}",
              flush=True)
    print(f"\n{len(unguarded)} constant(s) no test reacts to at 4x:", flush=True)
