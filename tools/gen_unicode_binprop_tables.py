...
def _emit(tables, version, path):
    ...
    for name in names:
        ...
        out += ["  };", ""]
    out += [f"  //! \\brief A Unicode binary property: the {len(names)} standard yes/no properties this build knows."]
    ...
    out += ["  struct binprop_alias_entry { std::string_view name; binprop prop; };", ""]
