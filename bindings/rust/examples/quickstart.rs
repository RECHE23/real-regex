// quickstart.rs — the landing page's Rust quickstart tab (docs/site/_templates/landing.html,
// doc-site P1c), built and run by `rust-test` (`cargo run --example quickstart`) against the
// crate so the code the page shows is exactly the code that's tested, never merely illustrative
// (doc-site P1b-A gate-snippet). A pair of region-boundary comments further down bounds the
...
// comments literally — the hook's own region search is a first-occurrence substring search, the
// same start-after/end-before contract Sphinx `literalinclude` used before P1c, and a mention up
// here would shadow the real one below) — do not edit the marked lines without also checking the
