...
guarantee, not a maybe. (`\p{L}` / `\p{sc=Greek}` / `\p{scx=Grek}` / `\p{Alphabetic}` — General_Category,
Script, Script_Extensions and the standard binary properties — *are* supported, a superset of stdlib
`re`.) Opt
...
[Differences from Python re](https://github.com/RECHE23/real-regex/blob/main/docs/site/differences-from-re.md).
