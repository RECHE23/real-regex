...
def validate(orbits, cased):
    ...
    for cp in cased_sorted:
        ...
        if got != expected:
            mismatches += 1
            if mismatches <= 8:
                print(f"  MISMATCH U+{cp:04X} {chr(cp)!r}: re={sorted(hex(x) for x in got)} "
                      f"table={sorted(hex(x) for x in expected)}", file=sys.stderr)
    if mismatches:
        sys.exit(f"ABORT: {mismatches} orbit(s) disagree with re.IGNORECASE")
...
def emit(orbits, path):
    ...
    lines = [
        ...
        f" * own case-insensitive matching, so REAL's text-mode icase is identical to re.IGNORECASE",
        ...
        "",
        f"  //! \\brief The Unicode data version these orbits were generated from ({ver}).",
        f"  inline constexpr const char* unicode_fold_unidata_version {{\"{ver}\"}};",
        "",
    ...
    lines += [
        "  };",
        "",
        f"  //! \\brief Number of entries in \\ref unicode_fold_table.",
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"wrote {path}: {len(entries)} entries, {len(orbits)} orbits, Unicode {ver}")
