...
| Unicode property classes `\p{…}` | **supported** (General_Category + Script + the standard binary properties) | native, linear `klass_cp`; a superset of `re` (which rejects `\p`); other UAX44 properties (`Bidi_Class`, `Word_Break`, `Age`, …) stay `unsupported` | 2026.7 |
