...
both ISAs, on the same `1aa76c7` tree for this re-stamp.
...
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.67 | 28.91 (**17.32×**) | 6.42 (**3.85×**) | 28.32 (**16.97×**) |
| digits `[0-9]+` | 1.08 | 24.46 (**22.63×**) | 3.65 (**3.38×**) | 16.74 (**15.49×**) |
| fields `[^,]+` | 6.28 | 25.18 (**4.01×**) | 4.98 (0.79×) | 22.82 (**3.64×**) |
| alternation `the\|fox\|dog` | 2.45 | 29.80 (**12.17×**) | 2.26 (0.92×) | 10.44 (**4.26×**) |
| date `{4}-{2}-{2}` | 0.70 | 18.49 (**26.45×**) | 0.62 (0.89×) | 4.09 (**5.85×**) |
| hex `[0-9a-f]{8}` | 1.35 | 21.01 (**15.56×**) | 1.91 (**1.41×**) | 4.08 (**3.02×**) |
| literal | 0.43 | 15.03 (**35.20×**) | 0.60 (**1.41×**) | 2.38 (**5.57×**) |
| anchored `^[a-z]+$` | 0.52 | unsupported | 0.52 (**1.01×**) | 1.78 (**3.46×**) |
| lookahead `[a-z]+(?=[a-z])` | 7.62 | 76.60 (**10.05×**) | 6.83 (0.90×) | unsupported |

**arm64** — Apple M1 Pro, Apple clang 16, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 11.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.16 | 92.20 (**79.28×**) | 2.32 (**1.99×**) | 13.92 (**11.97×**) |
| digits `[0-9]+` | 0.85 | 81.61 (**96.47×**) | 1.44 (**1.70×**) | 8.44 (**9.98×**) |
| fields `[^,]+` | 3.23 | 75.03 (**23.21×**) | 1.87 (0.58×) | 11.05 (**3.42×**) |
| alternation `the\|fox\|dog` | 1.98 | 113.50 (**57.35×**) | 1.57 (0.79×) | 6.34 (**3.20×**) |
| date `{4}-{2}-{2}` | 0.49 | 71.61 (**145.85×**) | 0.39 (0.79×) | 3.41 (**6.95×**) |
| hex `[0-9a-f]{8}` | 1.42 | 79.33 (**55.94×**) | 1.24 (0.87×) | 3.41 (**2.40×**) |
| literal | 0.25 | 30.81 (**124.74×**) | 0.48 (**1.94×**) | 1.35 (**5.47×**) |
| anchored `^[a-z]+$` | 0.32 | unsupported | 0.42 (**1.32×**) | 2.19 (**6.87×**) |
| lookahead `[a-z]+(?=[a-z])` | 4.00 | 155.92 (**38.93×**) | 3.66 (0.91×) | unsupported |
...
**Stamp.** REAL `2026.8.6+` (tree `1aa76c7`), both ISAs re-measured for this stamp, on the same
...
**3.482** (pcre2 **1.00×**, re2 **1.43×**), `\p{L}+` CJK **3.376** (0.84× / re2 **5.69×**), `\p{N}+`
**4.484** (0.57× / **1.67×**), `sc=Han` **5.737** (0.81×), `scx=Cyrl` **6.531** (0.99×), `(?i)café`
**0.673** (**0.89×** / **4.18×**), `[à-ÿ]+` **3.551** (**1.66×** / **7.13×**), literal `你好` **0.817**
(pcre2 **1.26×** — the same crossing as arm64), `.` emoji **8.224** (pcre2 **1.26×**), ascii witness
**1.669** (**3.85×**). Oracle: exhaustive `\p{L}` over U+0000..10FFFF (surrogates skipped) —
**0 mismatch**.
...
| --- | ---: | ---: | ---: | ---: | --- |
| `\w+` (mixed-script) | 1.932 | 60.21 (**31.16×**) | 1.86 (0.96×) | 3.20 (**1.66×**) | 16218/5406/16218/5406 ⚠ |
| `\p{L}+` (CJK) | 1.951 | unsupported | 1.33 (0.68×) | 13.43 (**6.88×**) | 12904/—/12904/12904 |
| `\p{N}+` (arabic digits) | 1.873 | unsupported | 1.61 (0.86×) | 5.20 (**2.78×**) | 6250/—/6250/6250 |
| `\p{sc=Han}` (CJK) | 2.285 | unsupported | 2.14 (0.94×) | unsupported | 25808/—/25808/— |
| `\p{scx=Cyrl}` (mixed-script) | 2.690 | unsupported | 2.69 (**1.00×**) | unsupported | 32436/—/32436/— |
| `(?i)café` (accented) | 0.404 | unsupported | 0.34 (0.84×) | 1.30 (**3.22×**) | 3509/—/3509/3509 |
| `[à-ÿ]+` (accented) | 1.574 | 86.61 (**55.03×**) | 2.06 (**1.31×**) | 12.57 (**7.99×**) | 38599/38599/38599/38599 |
| literal `你好` (CJK) | 0.457 | 28.95 (**63.35×**) | 0.58 (**1.27×**) | 2.62 (**5.73×**) | 6452/6452/6452/6452 |
| `.` (emoji, one codepoint) | 4.771 | 58.32 (**12.22×**) | 3.78 (0.79×) | 18.70 (**3.92×**) | 68306/200039/68306/68306 ⚠ |
| ascii witness `[a-z]+` | 1.164 | 91.87 (**78.93×**) | 2.25 (**1.93×**) | 13.90 (**11.94×**) | 42108/42108/42108/42108 |
