...
both ISAs, on the same `5fbbff1` tree for this re-stamp.
...
The x86-64 set is tight — every row inside 1 % across the three runs except `fields` (6.198 / 6.209 /
6.366) and `literal` (0.437 / 0.437 / 0.465). On arm64 the three runs hold inside 1 % on `words`,
`hex`, `literal` and `lookahead`; `date` spreads 4.9 % (0.488 / 0.501 / 0.512) and `anchored` 7.8 %
(0.319 / 0.331 / 0.344) — that row matches **once** over the whole corpus, so its cell is one scan
rather than an average of thousands, and it is the least stable number in the table on either ISA.

**Two sets were measured and discarded before these, both caught rather than published.** An earlier
arm64 set ran while the machine was building, and it reads slower on exactly the rows that scan the
longest — `digits` 0.873–0.900 against 0.846–0.855 here, `lookahead` 3.931–3.997 against 3.820–3.852.
Contamination inflates, so a contaminated run cannot be mixed into a minimum. And an x86-64 set
linked **PCRE2 10.42**, the container's system copy, instead of the pinned 10.47: that alone moved
`anchored`'s PCRE2 cell from 0.52 to 0.77 (a 1.01× that would have been published as 1.50× in REAL's
favour). The harness prints the version it LINKED for this reason — the trap has now been caught
twice by that one line.

**x86-64** — devbox, g++ 13.3.0, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 10.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.46 | 29.34 (**20.05×**) | 6.43 (**4.40×**) | 28.23 (**19.30×**) |
| digits `[0-9]+` | 1.03 | 24.56 (**23.75×**) | 3.64 (**3.52×**) | 16.79 (**16.24×**) |
| fields `[^,]+` | 6.21 | 25.10 (**4.04×**) | 4.94 (0.80×) | 22.88 (**3.68×**) |
| alternation `the\|fox\|dog` | 2.30 | 30.66 (**13.31×**) | 2.25 (0.98×) | 10.38 (**4.51×**) |
| date `{4}-{2}-{2}` | 0.68 | 18.52 (**27.08×**) | 0.61 (0.89×) | 4.09 (**5.98×**) |
| hex `[0-9a-f]{8}` | 1.36 | 20.99 (**15.40×**) | 1.91 (**1.40×**) | 4.08 (**2.99×**) |
| literal | 0.44 | 15.09 (**34.61×**) | 0.59 (**1.35×**) | 2.38 (**5.46×**) |
| anchored `^[a-z]+$` | 0.51 | unsupported | 0.52 (**1.01×**) | 1.78 (**3.46×**) |
| lookahead `[a-z]+(?=[a-z])` | 7.73 | 76.57 (**9.91×**) | 6.79 (0.88×) | unsupported |

**arm64** — Apple M1 Pro, Apple clang 16, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 11.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.17 | 92.30 (**79.23×**) | 2.33 (**2.00×**) | 14.00 (**12.02×**) |
| digits `[0-9]+` | 0.85 | 83.92 (**99.20×**) | 1.44 (**1.70×**) | 8.53 (**10.08×**) |
| fields `[^,]+` | 3.19 | 74.95 (**23.50×**) | 1.86 (0.58×) | 11.06 (**3.47×**) |
| alternation `the\|fox\|dog` | 1.98 | 114.97 (**58.04×**) | 1.57 (0.79×) | 6.14 (**3.10×**) |
| date `{4}-{2}-{2}` | 0.49 | 72.79 (**149.16×**) | 0.39 (0.80×) | 3.42 (**7.01×**) |
| hex `[0-9a-f]{8}` | 1.42 | 81.35 (**57.45×**) | 1.25 (0.88×) | 3.47 (**2.45×**) |
| literal | 0.25 | 30.88 (**125.02×**) | 0.48 (**1.94×**) | 1.36 (**5.51×**) |
| anchored `^[a-z]+$` | 0.32 | unsupported | 0.42 (**1.32×**) | 2.19 (**6.87×**) |
| lookahead `[a-z]+(?=[a-z])` | 3.82 | 157.87 (**41.33×**) | 3.68 (0.96×) | unsupported |
...
| --- | ---: | ---: | ---: | ---: | --- |
| `\w+` (mixed-script) | 2.361 | 60.71 (25.71×) | 1.86 (**0.79×**) | 3.20 (1.36×) | 16218/5406/16218/5406 ⚠ |
| `\p{L}+` (CJK) | 2.868 | unsupported | 1.33 (**0.46×**) | 13.28 (4.63×) | 12904/—/12904/12904 |
| `\p{N}+` (arabic digits) | 1.909 | unsupported | 1.59 (**0.83×**) | 5.22 (2.73×) | 6250/—/6250/6250 |
| `\p{sc=Han}` (CJK) | 3.422 | unsupported | 2.15 (**0.63×**) | unsupported | 25808/—/25808/— |
| `\p{scx=Cyrl}` (mixed-script) | 2.697 | unsupported | 2.67 (**0.99×**) | unsupported | 32436/—/32436/— |
| `(?i)café` (accented) | 1.292 | unsupported | 0.34 (**0.26×**) | 1.30 (1.01×) | 3509/—/3509/3509 |
| `[à-ÿ]+` (accented) | 1.617 | 89.27 (55.21×) | 2.06 (**1.27×**) | 12.57 (7.77×) | 38599/38599/38599/38599 |
| literal `你好` (CJK) | 0.457 | 29.11 (63.70×) | 0.58 (**1.27×**) | 2.63 (5.75×) | 6452/6452/6452/6452 |
| `.` (emoji, one codepoint) | 4.622 | 59.37 (12.85×) | 3.78 (**0.82×**) | 18.74 (4.05×) | 68306/200039/68306/68306 ⚠ |
| ascii witness `[a-z]+` | 1.180 | 93.32 (79.08×) | 2.25 (**1.91×**) | 14.12 (11.97×) | 42108/42108/42108/42108 |
