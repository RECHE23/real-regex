...
both ISAs, on the same `3cdd13c` (v2026.8.2) tree for this re-stamp.

**Protocol for this re-stamp:** three full harness runs per ISA, each already a median of N = 30
paired batches, then the **minimum per cell across the three**. One run is not enough on the x86-64
container, where interference arrives in episodes that contaminate contiguous rows (see Methodology);
`words` is the only row where the three runs spread more than 3 % (3.39 / 3.55 / 3.63). The PCRE2
version is the one the binary LINKED, now printed by the harness rather than asserted here.

**x86-64** — devbox, g++ 13.3.0, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 10.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 3.39 | 28.75 (**8.49×**) | 6.40 (**1.89×**) | 28.26 (**8.34×**) |
| digits `[0-9]+` | 1.89 | 24.24 (**12.83×**) | 3.63 (**1.92×**) | 16.81 (**8.89×**) |
| fields `[^,]+` | 6.03 | 24.79 (**4.11×**) | 4.93 (0.82×) | 22.95 (**3.80×**) |
| alternation `the\|fox\|dog` | 2.32 | 30.14 (**12.98×**) | 2.26 (0.97×) | 10.39 (**4.47×**) |
| date `{4}-{2}-{2}` | 0.71 | 17.93 (**25.11×**) | 0.61 (0.85×) | 4.09 (**5.73×**) |
| hex `[0-9a-f]{8}` | 1.37 | 20.28 (**14.81×**) | 1.89 (**1.38×**) | 4.09 (**2.99×**) |
| literal | 0.43 | 14.99 (**34.78×**) | 0.59 (**1.37×**) | 2.38 (**5.52×**) |
| lookahead `[a-z]+(?=[a-z])` | 8.08 | 75.98 (**9.40×**) | 6.77 (0.84×) | unsupported |

**arm64** — Apple M1 Pro, Apple clang 16, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 11.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 2.35 | 91.69 (**39.03×**) | 2.33 (0.99×) | 14.08 (**5.99×**) |
| digits `[0-9]+` | 1.44 | 82.80 (**57.34×**) | 1.44 (1.00×) | 8.48 (**5.87×**) |
| fields `[^,]+` | 3.24 | 75.15 (**23.17×**) | 1.87 (0.58×) | 11.04 (**3.40×**) |
| alternation `the\|fox\|dog` | 1.98 | 112.41 (**56.89×**) | 1.56 (0.79×) | 6.14 (**3.11×**) |
| date `{4}-{2}-{2}` | 0.50 | 72.68 (**145.07×**) | 0.39 (0.78×) | 3.41 (**6.81×**) |
| hex `[0-9a-f]{8}` | 1.42 | 82.15 (**57.69×**) | 1.24 (0.87×) | 3.41 (**2.39×**) |
| literal | 0.25 | 30.91 (**125.14×**) | 0.48 (**1.94×**) | 1.36 (**5.51×**) |
| lookahead `[a-z]+(?=[a-z])` | 4.24 | 158.62 (**37.42×**) | 3.66 (0.86×) | unsupported |

**Reading — verdict brut, no dressing up.**

- **REAL ≫ `std::regex`**, always: 4.1–34.8× on x86-64, 23.2–145.1× on arm64 (libc++'s
  `std::regex` falls even further behind on arm64). Never below 4.1×.
- **REAL > RE2**, always where RE2 supports the pattern: 3.0–8.9× on x86-64, 2.4–6.8× on arm64.
- **REAL vs PCRE2-JIT is close, ISA-dependent, and mixed.** REAL leads on `words` (**1.89×** x86-64 /
  0.99× arm64), `digits` (**1.92×** / 1.00× — an exact tie on arm64), `hex` (**1.38×** / 0.87×)
  and `literal` (**1.37×** / **1.94×**, its only two-ISA win). PCRE2 keeps `fields` (0.82× / 0.58×),
  `date` (0.85× / 0.78×), `lookahead` (0.84× / 0.86×) and `alternation` (0.97× / 0.79×).
- **Two rows moved because two routing defects were fixed**, and they are the only rows whose change
  is attributable to this train. **`date`** was building the general VM for a fixed-shape pattern the
  inner-literal gate had already claimed: **1.17 → 0.71 ns/B on x86-64 (−39 %)** and **0.76 → 0.50
  on arm64 (−34 %)**, taking the PCRE2 ratio from 0.59×/0.51× to 0.85×/0.78×. **`lookahead`** was
  re-resolving an invariant trailing lookaround at every candidate end position instead of once per
  walk: **8.95 → 8.08 (−9.7 %)** and **6.18 → 4.24 (−31 %)**, 0.81×/0.59× to 0.84×/0.86×. Neither
  is now REAL's row — both are still PCRE2 ground — but neither is a rout any more.
- **Two rows moved because earlier trains were never re-stamped here**, which is a failure of this
  document rather than a finding: `words` **4.15 → 3.39 (−18 %)** and `digits` **2.36 → 1.89
  (−20 %)** on x86-64, both from the code-point-class scan work landed several versions back. Their
  arm64 twins did not move. Two of the four rows that read as gains here are simply the table
  catching up.
- **`fields` got slower and is not explained away.** **5.49 → 6.03 on x86-64 (+9.8 %)** and **3.11 →
  3.24 on arm64 (+4.2 %)** — both outside the ±3 % floor, both in the same direction on two ISAs,
  which is what separates this from layout noise. The previous stamp attributed an earlier `fields`
  move to a layout effect; this one is not attributed, because nothing measured supports it yet. What
  is known: the row is on the **code-point** route, not the byte class loop, and a byte-level
  substitution is NOT equivalent — see the Methodology note. Open, and named as open.
- **`alternation` crossed back on x86-64**, 1.02× → **0.97×**. The previous stamp called that
  crossing "a bare win", and a bare win is what a row inside the noise looks like from the winning
  side. Recorded here as the correction it is, not left to be discovered by the next re-stamp.
- **The lookahead line stays in PCRE2-JIT's order of magnitude** — **8.08 / 4.24 ns/B** against the
  pre-P3c general-VM order (~92 / ~48). REAL does a **bounded lookaround in linear time**; PCRE2 is
  faster here but by **backtracking** (itself ReDoS-able on a crafted lookaround), and **RE2 and the
  rust crate cannot compile the pattern at all** (`unsupported`). `find_iter` / Python `finditer` do
  not get the P3c fast path by construction (return type fixed at compile time so pure `[a-z]+` does
  not regress) — this row is `count_matches` only, stated plainly so it is not read as a `find_iter`
  number.
- **The gauge that makes the above readable:** `std::regex` and RE2 are third-party constants here,
  so their columns are the drift witness. Against the previous stamp they land within **1 %** on
  x86-64 on every row (RE2 28.32 → 28.26, 16.73 → 16.81, 22.79 → 22.95, 10.39 → 10.39, 4.09 → 4.09,
  2.41 → 2.38), which is what licenses reading a −39 % REAL row as REAL's.
