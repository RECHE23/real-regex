...
both ISAs, on the same `2ac7601` (v2026.8.2) tree for this re-stamp.
...
three x86-64 rows spread past 3 % across the runs and the minimum is what keeps them readable — `lookahead` 7.63 / 7.75 / 8.04, `fields` 6.09 / 6.24 / 6.35, `date` 0.711 / 0.718 / 0.736; `alternation` is the tightest at 0.7 %. The PCRE2
...
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 2.82 | 28.81 (**10.23×**) | 6.41 (**2.28×**) | 28.24 (**10.02×**) |
| digits `[0-9]+` | 1.81 | 24.43 (**13.48×**) | 3.63 (**2.00×**) | 16.89 (**9.32×**) |
| fields `[^,]+` | 6.09 | 25.21 (**4.14×**) | 4.94 (0.81×) | 22.74 (**3.73×**) |
| alternation `the\|fox\|dog` | 2.23 | 29.77 (**13.36×**) | 2.26 (**1.01×**) | 10.39 (**4.66×**) |
| date `{4}-{2}-{2}` | 0.71 | 18.67 (**26.26×**) | 0.61 (0.86×) | 4.08 (**5.74×**) |
| hex `[0-9a-f]{8}` | 1.35 | 21.25 (**15.74×**) | 1.93 (**1.43×**) | 4.08 (**3.02×**) |
| literal | 0.41 | 15.21 (**36.92×**) | 0.59 (**1.43×**) | 2.38 (**5.78×**) |
| lookahead `[a-z]+(?=[a-z])` | 7.63 | 76.74 (**10.06×**) | 6.78 (0.89×) | unsupported |

**arm64** — Apple M1 Pro, Apple clang 16, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 11.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 2.26 | 93.16 (**41.28×**) | 2.36 (**1.05×**) | 14.09 (**6.24×**) |
| digits `[0-9]+` | 1.41 | 83.59 (**59.41×**) | 1.46 (**1.04×**) | 8.58 (**6.10×**) |
| fields `[^,]+` | 3.24 | 74.92 (**23.15×**) | 1.90 (0.59×) | 11.19 (**3.46×**) |
| alternation `the\|fox\|dog` | 1.92 | 113.21 (**59.09×**) | 1.59 (0.83×) | 6.25 (**3.26×**) |
| date `{4}-{2}-{2}` | 0.53 | 72.67 (**138.42×**) | 0.39 (0.74×) | 3.43 (**6.53×**) |
| hex `[0-9a-f]{8}` | 1.42 | 81.26 (**57.06×**) | 1.24 (0.87×) | 3.39 (**2.38×**) |
| literal | 0.25 | 31.43 (**126.73×**) | 0.48 (**1.94×**) | 1.36 (**5.48×**) |
| lookahead `[a-z]+(?=[a-z])` | 3.85 | 158.98 (**41.29×**) | 3.71 (0.96×) | unsupported |

**Reading — verdict brut, no dressing up.**

- **REAL ≫ `std::regex`**, always: 4.1–36.9× on x86-64, 23.2–138.4× on arm64 (libc++'s
  `std::regex` falls even further behind on arm64). Never below 4.1×.
- **REAL > RE2**, always where RE2 supports the pattern: 3.0–10.0× on x86-64, 2.4–6.5× on arm64.
- **REAL vs PCRE2-JIT: four rows are REAL's on both ISAs now**, where the previous stamp had one.
  `words` (**2.28×** x86-64 / **1.05×** arm64), `digits` (**2.00×** / **1.04×**), `literal`
  (**1.43×** / **1.94×**), and `hex` on x86-64 (**1.43×**, 0.87× on arm64). `alternation` crossed on
  x86-64 (**1.01×**) and stays PCRE2's on arm64 (0.83×). PCRE2 keeps `fields` (0.81× / 0.59×), `date`
  (0.86× / 0.74×) and `lookahead` (0.89× / 0.96×) — the last of those now within 4 % on arm64.
- **What moved these rows is one accessor, and it moved every route that shares it.** `class_table`
  ...
  exactly 3.00. Asking the key first and outlining the rest: **`lookahead` −9.2 % arm64 / −5.6 %
  x86-64**, **`words` −3.8 % / −16.8 %**, `alternation` −3.0 % / −3.9 %, `digits` −2.1 % / −4.2 %,
  `literal` flat / −4.7 %. Isolated per-route gauges agree on direction for every one of them.
- **`fields` did not move, and that is now the finding rather than the mystery.** It was published as
  ...
  moves every other row measures **−25 % on x86-64 in the probe and +1.0 % in this table**. Two
...
- **The lookahead line is now within 4 % of PCRE2-JIT on arm64** (0.96×) and 0.89× on x86-64, against
  the pre-P3c general-VM order (~92 / ~48 ns/B). REAL does a **bounded lookaround in linear time**;
