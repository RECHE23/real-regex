...
| | |
| --- | --- |
| Version | REAL `2026.7.58` — **re-measured for this stamp: §E.4 only** (the crate's own criterion rows, arm64, at `7ee487c`), by an interleaved A/B against `v2026.7.57` with the same bench file on both sides, one group at a time, machine otherwise idle, and `compile` carried as a control throughout — nothing in this train touches it and it stays within 1.8 %. Two rounds for `find`/`compile`/`first_use`; **four rounds of 5 s for `captures`**, which was not decoration: `captures/digits` read +8.7 % on two rounds and +2.1 % on four, with per-round dispersion falling to 0.1 %. **The row this train was about:** `\b\w+\b` had been the largest tracked scan deficit for three releases, disclosed and not chased each time — `find/word_bound` **317.8 → 261.6 µs (−17.7 %)**, 1.90× behind the `regex` crate becomes **1.56×**, and `captures/word_bound` **453.2 → 318.5 (−29.7 %)**, 2.71× becomes **1.91×**, without touching `\b`: the three membership accessors were being emitted out of line (`class_table` 6.23 M instructions against 1.44 M inlined), the per-regex row flags were three allocations where one atomic word does, and the per-`run()` program-identity compare — per MATCH on a walk — now folds away at compile time for callers that own their state. Direct harness, 64 KiB walk, arm64: `\d+` **−40.1 %**, `\w+` and `\b\w+\b` **−17.7 %**. **What it costs:** `first_use/fields` +4.1 %, `/digits` +3.6 %, `/class` +3.3 %; `captures/class` +2.6 %, `find/digits` +2.5 %, `captures/digits` +2.1 %; `[a-z]+` walk +2.8 % on arm64 and **−3.7 %** in x86-64 instructions — the one row where the two ISAs disagree in sign, and the residual this train did not close. `dog` neutral over 12 runs; every other row within ±1.7 %. §A, §Unicode, §B and §multi-pattern carry their earlier figures unchanged. Per-train benchmark-impact log: CHANGELOG.md (full release notes: docs/release-notes/ + GitHub Releases). **Disclosed, not chased:** `(\w+)@(\w+)` `first_use` is still **4.57× behind** (2.78 ms against 609 µs) — the lazily built one-pass table is untouched by this train. **Caught while stamping, not before:** three cache fields were first added mid-struct in `basic_pike_state` and cost `find/word_bound` **+32.3 %** then +28.4 % on a second round, on a pattern reading none of them — a layout effect visible only through the C ABI, which crosses per match where the C++ harnesses call the engine directly. Fixed in `21ca882`; `pattern_hints` already says "appended last" five times, for the second release running. |
