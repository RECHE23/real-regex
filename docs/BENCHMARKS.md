...
| | |
| --- | --- |
| Version | REAL `2026.7.40` — §A/§E re-stamped on `a994ff9` (post O2r-1b, the A2 fix, the P0 #2 icase fix + seed-128 tune, and the mono/multi cascade split), 2026-07-11; §Unicode's `.`/ascii-witness cells re-stamped on `258783b` (post wagon-4 strict-UTF-8 fix for `.`), 2026-07-11 — every section carrying a re-stamp note is now re-stamped within this same train, not a stale carry-over. 7.42 (compat reorganized under `real/compat/` + the RE2 drop-in, `real/compat/re2/` — `FullMatch`/`PartialMatch`/`Consume`/`FindAndConsume`/`Replace`/`GlobalReplace`/`QuoteMeta`/`Arg`/`Options`/`Set` — + `\C` the bytes-mode raw-byte escape) adds no case to this suite and touches no §A/§E/§B/§Unicode/§multi-pattern measured code path — a structural include move, a new header-only zero-dep compat surface with no hot-path wiring of its own, and a bytes-mode-only escape reusing the existing byte-klass opcode (zero new runtime wiring, confirmed via the meta-seam); reported in the v2026.7.42 release notes, not a re-stamp trigger. 7.41 (issue #3: the `{n,}` counted-quantifier fast path + the `small_set` cap 4→8 raise + its own layout-fix) adds no case to this suite; the EXISTING cases it measurably touches — the class_loop (`words [a-z]+`, `witness [a-z]+`) — shift ~5% FASTER (favorable: the figures below are now conservative, not a stale carry-over hiding a loss), not a re-stamp trigger for the suite as a whole — this is the first recent train to touch a measured §A cell at all; the prior trains below all disclosed a clean miss (see the v2026.7.41 release notes for the full finding). A dedicated §A re-measure arc will re-stamp properly; this note keeps the carry-over honest rather than silent in the meantime. 7.40 (a Go binding, `bindings/go/` v0.1, cgo over the frozen C ABI — plus the ABI's own `(NULL,0)`-everywhere fix) adds no case to this suite and touches no §A/§E/§B/§Unicode/§multi-pattern measured code path — a new binding surface with its own cold-build numbers (~8s arm64 / ~15s x86-64 native), not this suite's concern, reported in the v2026.7.40 release notes, not a re-stamp trigger. 7.39 (R4: C-ABI foundation — `real_match`/`real_find_iter_between`/`real_sub`, 14→20 functions — + Python `RegexSet` rewired onto the native fused `real::regex_set`) adds no case to this suite and touches no §A/§E/§B/§Unicode/§multi-pattern measured code path — bindings-frontier-only, zero engine changes; RegexSet's own before/after numbers (small-set ~28% faster, fused-eligible ~5-9×, shape-dependent) are a disclosed, separate finding reported in the v2026.7.39 release notes, not part of this baseline, not a re-stamp trigger. 7.38 (possessive-capture-fix + R2 typed `class_ref`/byte-possessive-capture arming + R3 shape-envelope extraction) adds no case to this suite; the byte-possessive route (`a++`-style) and its now-armed captured variant are a disclosed, separate finding — not part of this baseline — measured directly by their own route-toggle A/B (arm64 + x86-64) and reported in the v2026.7.38 release notes, not a re-stamp trigger. 7.37 (triage-fixes: 4 correctness bugs + a CPython oracle-bug filter) adds no case to this suite and touches no measured code path — a route-toggle A/B (auto vs forced-general) confirms D1-perf's own possessive-loop ratios are unaffected by the new B-1 window-edge / same-table guards (see the v2026.7.37 release notes), not a re-stamp trigger. 7.36 (D1-perf Étage A, possessive fast-path routes) adds no case to this suite; the ONE existing case it measurably touches — "unicode . (emoji, one codepoint)" — is a disclosed, isolated ~5% arm64 residual (reversed to a gain on x86-64), not a re-stamp trigger for the suite as a whole (see the v2026.7.36 release notes for the full finding); the possessive-vs-greedy win itself is a separate, disclosed finding, not part of this baseline. 7.35 (D1, possessive quantifiers / atomic groups) adds no case to this suite and touches no EXISTING measured code path — confirmed flat by its own interleaved-binaries A/B, not a re-stamp trigger. 7.34 (the P1 `\b`/`\B`-junction fix) touches no measured code path — confirmed flat by its own dual-ISA A/B, not a re-stamp trigger. §multi-pattern unchanged (Stage-2 + Arc I/II + Arc B `\b` unlock) |
...
both ISAs, on the same `a994ff9` tree.

**x86-64** — devbox, g++ 13.3.0, N = 30, PCRE2 10.47-JIT, RE2 10.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 3.72 | 29.22 (7.86×) | 6.41 (**1.72×**) | 28.27 (7.60×) |
| digits `[0-9]+` | 2.08 | 24.40 (11.73×) | 3.79 (**1.82×**) | 16.87 (8.11×) |
| fields `[^,]+` | 4.53 | 25.48 (5.63×) | 4.95 (**1.09×**) | 22.92 (5.06×) |
| alternation `the\|fox\|dog` | 2.33 | 30.06 (12.90×) | 2.25 (0.96×) | 10.38 (4.46×) |
| date `{4}-{2}-{2}` | 1.13 | 18.79 (16.70×) | 0.62 (0.55×) | 4.08 (3.63×) |
| hex `[0-9a-f]{8}` | 1.42 | 20.57 (14.50×) | 1.82 (**1.29×**) | 4.08 (2.88×) |
| literal | 0.71 | 15.15 (21.26×) | 0.58 (0.82×) | 2.37 (3.33×) |
| lookahead `[a-z]+(?=[a-z])` | 9.52 | 76.56 (8.05×) | 6.64 (0.70×) | unsupported |

**arm64** — Apple M1 Pro, Apple clang 16, N = 30, PCRE2 10.47-JIT, RE2 11.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 2.41 | 92.73 (38.44×) | 2.33 (0.97×) | 13.93 (5.78×) |
| digits `[0-9]+` | 1.43 | 82.88 (57.97×) | 1.49 (**1.04×**) | 8.66 (6.06×) |
| fields `[^,]+` | 3.01 | 75.37 (25.05×) | 2.05 (0.68×) | 11.80 (3.92×) |
| alternation `the\|fox\|dog` | 2.14 | 116.17 (54.21×) | 1.65 (0.77×) | 6.33 (2.95×) |
| date `{4}-{2}-{2}` | 0.75 | 73.24 (98.27×) | 0.40 (0.53×) | 3.47 (4.66×) |
| hex `[0-9a-f]{8}` | 1.43 | 81.69 (57.01×) | 1.25 (0.87×) | 3.48 (2.43×) |
| literal | 0.61 | 30.87 (50.24×) | 0.50 (0.81×) | 1.36 (2.22×) |
| lookahead `[a-z]+(?=[a-z])` | 7.47 | 157.26 (21.05×) | 3.68 (0.49×) | unsupported |

**Reading — verdict brut, no dressing up.**

- **REAL ≫ `std::regex`**, always: 5.6–21.3× on x86-64, 21.1–98.3× on arm64 (libc++'s `std::regex` falls
  even further behind on arm64). Never below 5.6×.
- **REAL > RE2**, always where RE2 supports the pattern: 2.9–8.1× on x86-64, 2.2–6.1× on arm64.
- **REAL vs PCRE2-JIT is close, ISA-dependent, and mixed — this changed since the last stamp (2026.7.26),
  not just the numbers but the shape.** That snapshot reported an "alternation crosses PCRE2 on both ISAs"
  story (a SIMD candidate arc); on this tree alternation is 0.96× x86-64 (a near-tie, technically a loss)
  and 0.77× arm64 (a clearer loss) — **no longer a crossing on either ISA**. Whether that is the SIMD arc's
  own regression, a later hot-path change interacting with it (several landed since: O2r-1b, the A2 fix,
  the P0 #2 icase fix, the mono/multi cascade split), or something else is **not diagnosed here** — this is
  a measurement pass, not an investigation, and the fix history above did not touch `run_alternation`
  itself. Flagged for a look, not fixed. On the rows that still lead: x86-64 REAL is ahead on `words`
  (1.72×), `digits` (1.82×), `fields` (1.09×), `hex` (1.29×); arm64 only `digits` crosses (1.04×, thin).
  `date` and `literal` remain PCRE2 ground on both ISAs (0.53×–0.82×), consistent with the prior snapshot.
- **The lookahead line is transformed by P3c**, which landed since the last §A stamp. Matching-only
  throughput (`count_matches`, the equitable comparison against PCRE2/RE2's counters) went from the
  pre-P3c general-VM order (~92 ns/B x86-64, ~48 ns/B arm64) to **9.52 / 7.47 ns/B** — REAL is now in
  PCRE2-JIT's own order of magnitude (0.70× / 0.49×) instead of 10–30× behind it. REAL does a **bounded
