...
both ISAs, on the same `af10f1a` tree for this re-stamp.

**One row moved a long way and four x86-64 rows paid for it, and both figures are given.** The icase
fold change (see §Unicode) takes `(?i)café` from 0.26× to **0.84×** against PCRE2-JIT on arm64 and
0.29× to **0.89×** on x86-64. arm64 pays nothing for that — every other row within 0.2 %. gcc/x86-64
pays **+16.8 % on `words`**, +16.9 % on its ASCII witness and on `digits`, +15.3 % on `[à-ÿ]+`, which
is why those cells are worse here than in the previous stamp. **In a translation unit that includes
only `real.hpp` — the shape a user actually compiles — the same change costs `words` +4.6 % and
`[à-ÿ]+` +3.9 %, for the same −64 %.** The difference is this harness: it compiles `<regex>`, PCRE2,
RE2 and `real.hpp` together on gcc's per-unit inline budget, and the same unit was measured this
train charging +27.7 % for one added *data row*. The table publishes what the harness measures, the
sentence before this one publishes what a caller gets, and neither is omitted in favour of the other.
...
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.67 | 29.33 (**17.52×**) | 6.41 (**3.83×**) | 28.41 (**16.97×**) |
| digits `[0-9]+` | 1.08 | 24.50 (**22.66×**) | 3.63 (**3.36×**) | 16.84 (**15.58×**) |
| fields `[^,]+` | 6.18 | 25.50 (**4.13×**) | 4.95 (0.80×) | 22.96 (**3.72×**) |
| alternation `the\|fox\|dog` | 2.38 | 30.00 (**12.60×**) | 2.25 (0.94×) | 10.45 (**4.39×**) |
| date `{4}-{2}-{2}` | 0.75 | 18.42 (**24.69×**) | 0.61 (0.82×) | 4.22 (**5.66×**) |
| hex `[0-9a-f]{8}` | 1.36 | 20.64 (**15.22×**) | 1.90 (**1.40×**) | 4.11 (**3.03×**) |
| literal | 0.44 | 15.07 (**34.17×**) | 0.59 (**1.34×**) | 2.38 (**5.40×**) |
| anchored `^[a-z]+$` | 0.52 | unsupported | 0.52 (**1.01×**) | 1.78 (**3.46×**) |
| lookahead `[a-z]+(?=[a-z])` | 7.68 | 77.19 (**10.05×**) | 6.80 (0.89×) | unsupported |

**arm64** — Apple M1 Pro, Apple clang 16, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 11.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.16 | 91.96 (**79.07×**) | 2.32 (**1.99×**) | 14.01 (**12.05×**) |
| digits `[0-9]+` | 0.85 | 82.20 (**97.16×**) | 1.44 (**1.70×**) | 8.50 (**10.05×**) |
| fields `[^,]+` | 3.27 | 74.78 (**22.90×**) | 1.87 (0.57×) | 11.05 (**3.38×**) |
| alternation `the\|fox\|dog` | 1.93 | 113.58 (**58.82×**) | 1.56 (0.81×) | 6.14 (**3.18×**) |
| date `{4}-{2}-{2}` | 0.49 | 72.84 (**149.26×**) | 0.39 (0.80×) | 3.41 (**6.99×**) |
| hex `[0-9a-f]{8}` | 1.41 | 80.40 (**56.86×**) | 1.24 (0.88×) | 3.40 (**2.40×**) |
| literal | 0.26 | 30.89 (**117.45×**) | 0.48 (**1.83×**) | 1.36 (**5.17×**) |
| anchored `^[a-z]+$` | 0.32 | unsupported | 0.42 (**1.32×**) | 2.18 (**6.83×**) |
| lookahead `[a-z]+(?=[a-z])` | 3.82 | 155.78 (**40.80×**) | 3.66 (0.96×) | unsupported |
...
**Stamp.** REAL `2026.8.6+` (tree `af10f1a`), both ISAs re-measured for this stamp, on the same
...
**3.412** (pcre2 **1.02×**, re2 **1.47×**), `\p{L}+` CJK **3.399** (0.83× / re2 **5.68×**), `\p{N}+`
**4.273** (0.60× / **1.76×**), `sc=Han` **5.713** (0.80×), `scx=Cyrl` **6.407** (**1.01×**), `(?i)café`
**0.686** (0.87× / **4.07×**), `[à-ÿ]+` **3.436** (**1.70×** / **7.48×**), literal `你好` **0.844**
(pcre2 **1.22×** — the same crossing as arm64), `.` emoji **7.855** (pcre2 **1.31×**), ascii witness
**1.657** (**3.87×**). Oracle: exhaustive `\p{L}` over U+0000..10FFFF (surrogates skipped) —
**0 mismatch**.
...
| `\p{L}+` (CJK) | 1.954 | unsupported | 1.33 (0.68×) | 13.16 (**6.73×**) | 12904/—/12904/12904 |
| `\p{N}+` (arabic digits) | 1.873 | unsupported | 1.60 (0.85×) | 5.22 (**2.79×**) | 6250/—/6250/6250 |
| `\p{sc=Han}` (CJK) | 2.273 | unsupported | 2.14 (0.94×) | unsupported | 25808/—/25808/— |
| `\p{scx=Cyrl}` (mixed-script) | 2.730 | unsupported | 2.69 (0.99×) | unsupported | 32436/—/32436/— |
| `(?i)café` (accented) | 0.410 | unsupported | 0.34 (0.83×) | 1.30 (**3.17×**) | 3509/—/3509/3509 |
| `[à-ÿ]+` (accented) | 1.579 | 87.12 (**55.17×**) | 2.06 (**1.30×**) | 12.52 (**7.93×**) | 38599/38599/38599/38599 |
| literal `你好` (CJK) | 0.467 | 29.09 (**62.29×**) | 0.58 (**1.24×**) | 2.62 (**5.61×**) | 6452/6452/6452/6452 |
| `.` (emoji, one codepoint) | 4.697 | 58.28 (**12.41×**) | 3.74 (0.80×) | 18.74 (**3.99×**) | 68306/200039/68306/68306 ⚠ |
| ascii witness `[a-z]+` | 1.164 | 91.68 (**78.76×**) | 2.25 (**1.93×**) | 13.89 (**11.93×**) | 42108/42108/42108/42108 |
