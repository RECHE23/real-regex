...
| Python | CPython 3.14.6, `re` (stdlib) vs the in-place REAL `2026.8.11` extension (§B re-measured at this stamp and every row within 2 % of the previous one — see §B for why an engine-scan train should not move this table, and for the null calibration of its instrument; arm64 M1 Pro, N = 40 paired samples, bootstrap CI) |
...
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.79 | 28.94 (**16.15×**) | 6.41 (**3.58×**) | 28.13 (**15.70×**) |
| digits `[0-9]+` | 1.34 | 24.11 (**17.97×**) | 3.63 (**2.70×**) | 16.80 (**12.52×**) |
| fields `[^,]+` | 2.86 | 24.57 (**8.58×**) | 4.93 (**1.72×**) | 22.88 (**7.99×**) |
| single `[a-z]` | 5.07 | 66.23 (**13.06×**) | 23.83 (**4.70×**) | 91.16 (**17.98×**) |
| words `[a-z]{4,}` | 1.35 | 23.78 (**17.59×**) | 4.12 (**3.05×**) | 17.19 (**12.71×**) |
| words `[a-z]++` | 1.79 | 30.47 (**17.01×**) | 6.42 (**3.58×**) | unsupported |
| alternation `the\|fox\|dog` | 1.87 | 30.00 (**16.01×**) | 2.28 (**1.22×**) | 10.38 (**5.54×**) |
| date `{4}-{2}-{2}` | 0.74 | 17.98 (**24.40×**) | 0.61 (0.83×) | 4.08 (**5.54×**) |
| hex `[0-9a-f]{8}` | 1.36 | 20.36 (**14.95×**) | 1.90 (**1.40×**) | 4.08 (**3.00×**) |
| literal | 0.46 | 14.99 (**32.66×**) | 0.59 (**1.29×**) | 2.38 (**5.19×**) |
| anchored `^[a-z]+$` | 0.51 | unsupported | 0.52 (**1.01×**) | 1.78 (**3.46×**) |
| lookahead `[a-z]+(?=[a-z])` | 7.43 | 76.32 (**10.27×**) | 6.83 (0.92×) | unsupported |

**arm64** — Apple M1 Pro, Apple clang 16, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 11.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.26 | 92.70 (**73.51×**) | 2.32 (**1.84×**) | 13.88 (**11.01×**) |
| digits `[0-9]+` | 0.92 | 82.53 (**89.51×**) | 1.44 (**1.56×**) | 8.47 (**9.19×**) |
| fields `[^,]+` | 2.29 | 74.74 (**32.57×**) | 1.87 (0.81×) | 11.06 (**4.82×**) |
| single `[a-z]` | 4.47 | 66.66 (**14.90×**) | 8.12 (**1.82×**) | 41.33 (**9.24×**) |
| words `[a-z]{4,}` | 0.95 | 73.79 (**77.51×**) | 2.03 (**2.13×**) | 8.15 (**8.56×**) |
| words `[a-z]++` | 1.26 | unsupported | 2.33 (**1.85×**) | unsupported |
| alternation `the\|fox\|dog` | 1.70 | 114.65 (**67.44×**) | 1.57 (0.92×) | 6.15 (**3.62×**) |
| date `{4}-{2}-{2}` | 0.49 | 72.18 (**146.11×**) | 0.39 (0.79×) | 3.44 (**6.96×**) |
| hex `[0-9a-f]{8}` | 1.42 | 80.43 (**56.80×**) | 1.24 (0.88×) | 3.39 (**2.39×**) |
| literal | 0.25 | 30.99 (**125.47×**) | 0.48 (**1.94×**) | 1.35 (**5.47×**) |
| anchored `^[a-z]+$` | 0.32 | unsupported | 0.42 (**1.32×**) | 2.19 (**6.87×**) |
| lookahead `[a-z]+(?=[a-z])` | 3.81 | 159.31 (**41.80×**) | 3.69 (0.97×) | unsupported |
...
**RE-MEASURED AT `2026.8.11+`, AND THE TRAIN DID NOT MOVE IT — which withdraws a claim the v2026.8.11
release notes made.** Those notes said §B was one train stale and that "this train's batching changes
