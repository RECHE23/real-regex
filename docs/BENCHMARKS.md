...
| | |
| --- | --- |
| Version | REAL `2026.7.60` — **re-measured for this stamp: §E.4 only** (the crate's own criterion rows, arm64, at `69b99a3`), by an interleaved A/B against `v2026.7.59` with the same bench file on both sides — which is what lets this release's three NEW families exist on both — one group at a time, two rounds, machine otherwise idle. No row regresses by more than 3 %; every row other than the three `repeat` gains lands within ±2.2 %. **The row this train was about did not exist before it:** the suite had eight families and not one counted repeat (every family used `+`, `*` or a literal) and no case-insensitive pattern at all, while both compile-cost defects this project has shipped lived in the icase path. Three families were added for the second reason and immediately found something bigger, for the first — `find/repeat` (`\w{8}`) **2.24 ms → 123.9 µs (−94.5 %)**, and **24.55× behind the `regex` crate becomes 1.35×**; `captures/repeat` 24.46× → 1.33×; `first_use/repeat` 93.5× ahead → 113.4×. `\w+` and `\w{8}` look alike and are not: three shapes bracket the counted one and all three were already fast (`[a-z]{8}` via fixed_shape, `\w{8,}` and `\w+` via the code-point class loop), leaving exactly one combination unserved — a code-point class with a bounded count, which ran on the general Pike VM and grew superlinearly (247 µs for `\w+`, 499 for `\w{4}`, 2369 for `\w{8}`). arm64 walk: `\w{8}` **−95.1 %**, `\w{4}` −57.0 %, `\d{4}` −9.9 %, with `\w+`/`\w{8,}`/`[a-z]+`/`\b\w+\b` at 0.0 %; x86-64 `\w{8}` **−96.8 %** on the clock. **Compile cost, continued:** the case fold is now cached per (class, mode, negated) and holds the FINISHED class, so `\w{64}` with icase compiles in **0.128 ms against 3.84** (−96.7 %) and `\w{500}a` costs **−97.0 %** of its x86-64 instructions; `\w{k}a` grows sub-linearly in k where it was linear. §A, §Unicode, §B and §multi-pattern carry their earlier figures unchanged. Per-train benchmark-impact log: CHANGELOG.md (full release notes: docs/release-notes/ + GitHub Releases). **Disclosed, not chased:** `(?i)[a-z]+` scans **3.37× behind** the crate and 4.3× behind its own plain form — under icase `[a-z]` gains the long s and the Kelvin sign and stops being a pure ASCII class, while `(?i)dog` is 1.17× ahead, so it is the class widening and not the flag; visible for the first time here because the row did not exist. `(\w+)@(\w+)` first use stays **2.64× behind**. `\w{k}\b` keeps the general VM: the first version of the route lost it (`\w{4}\b` over "abcdefghi" matches at 5 and returned nothing) because both retry loops advance by the whole run on failure — right for a maximal run, wrong for a bounded one — and differencing the route against the general VM caught that before it shipped. **Disagreeing measurements, both stated:** on gcc/x86-64 the unbounded shapes read +2.3 % in INSTRUCTIONS (three branch orderings measured, all identical, so it is the code's presence and not its position) while the CLOCK reads −0.2 % for `\w+` and +1.3 % for `\b\w+\b`, and arm64 reads 0.0 % for both. |
