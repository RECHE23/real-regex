...
both ISAs, on the same `10aeafc` tree for this re-stamp.

**This stamp exists because a CORRECTNESS fix moved x86-64, and carrying the previous table over
would have published numbers already known to be wrong.** The train behind it fixes a
heap-use-after-free on a result outliving a temporary regex; giving the rvalue path its own result
type leaves the borrowing path — every walk, every attempt on a live regex — byte for byte what it
was, and arm64 is flat throughout (`words` 1.16 → 1.16, `digits` 0.85 → 0.85). gcc/x86-64 is not:
**`digits` 1.08 → 1.21** and `words` 1.67 → 1.76 here, against `alternation` 2.38 → 2.29, `date`
...
**And the arm64 half of that story is RETRACTED, by the same diagnostic.** Folding the owner into
the shared result type was published here as costing arm64 `words` **+5.6 %**, with its ASCII witness
at +5.6 % offered as proof it was "not layout noise". Rebuilt and re-measured on two independent
builds, it reads **−0.01 % and +0.03 %**, witness +0.02 % and +0.03 % — median +0.04 % across the
table, worst row +2.2 %. The only structural change the two binaries show is one out-of-line
destructor. So the original figure was one build's placement draw, and the witness argument was
**wrong on its own terms**: two instances of one pattern compile to the same code, so they move
together under a placement shift exactly as they would under a real cost. A same-pattern witness
separates per-row measurement noise; it does not separate placement from cost.

The two-result-type design is unchanged and still correct — for its API reason, which never depended
on a benchmark: a distinct type means no conversion can quietly drop the ownership and hand back the
dangling views it exists to prevent, and the borrowing path keeps a trivially destructible result
instead of carrying an owning box it can never use.
...
machine, and this set says so again: arm64 `words` reads 1.239 / 1.166 / 1.164 — 6.5 % — while
`digits`, `literal`, `anchored` and `hex` hold inside 0.2 % across the same three runs; x86-64 is
tighter but not clean either (`digits` 2.7 %, `anchored` 2.7 %, `hex` 0.1 %). That is the episode
shape described under Methodology: a burst that lands on whatever is scanning when it arrives, not a
property of the row. A minimum across runs is what survives it; a mean would not. **33 of the
published REAL cells came from a run other than the first** — 14 on arm64, 19 on x86-64 — which is
the protocol earning its cost rather than performing it.
...
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.76 | 29.72 (**16.89×**) | 6.42 (**3.65×**) | 28.33 (**16.10×**) |
| digits `[0-9]+` | 1.21 | 24.53 (**20.27×**) | 3.64 (**3.01×**) | 16.84 (**13.92×**) |
| fields `[^,]+` | 6.21 | 25.23 (**4.06×**) | 4.95 (0.80×) | 22.95 (**3.70×**) |
| alternation `the\|fox\|dog` | 2.29 | 30.45 (**13.30×**) | 2.28 (1.00×) | 10.49 (**4.58×**) |
| date `{4}-{2}-{2}` | 0.70 | 18.80 (**26.86×**) | 0.61 (0.87×) | 4.11 (**5.87×**) |
| hex `[0-9a-f]{8}` | 1.37 | 20.92 (**15.27×**) | 1.90 (**1.39×**) | 4.10 (**2.99×**) |
| literal | 0.43 | 14.97 (**34.81×**) | 0.59 (**1.37×**) | 2.39 (**5.56×**) |
| anchored `^[a-z]+$` | 0.51 | unsupported | 0.52 (**1.02×**) | 1.78 (**3.49×**) |
| lookahead `[a-z]+(?=[a-z])` | 7.70 | 76.28 (**9.91×**) | 6.79 (0.88×) | unsupported |

**arm64** — Apple M1 Pro, Apple clang 16, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 11.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.16 | 91.20 (**78.62×**) | 2.32 (**2.00×**) | 13.92 (**12.00×**) |
| digits `[0-9]+` | 0.85 | 81.70 (**96.12×**) | 1.44 (**1.69×**) | 8.52 (**10.02×**) |
| fields `[^,]+` | 3.21 | 73.71 (**22.96×**) | 1.87 (0.58×) | 10.97 (**3.42×**) |
| alternation `the\|fox\|dog` | 1.94 | 113.12 (**58.31×**) | 1.56 (0.80×) | 6.12 (**3.15×**) |
| date `{4}-{2}-{2}` | 0.48 | 71.70 (**149.38×**) | 0.39 (0.81×) | 3.41 (**7.10×**) |
| hex `[0-9a-f]{8}` | 1.42 | 79.69 (**56.12×**) | 1.25 (0.88×) | 3.41 (**2.40×**) |
| literal | 0.25 | 30.85 (**123.40×**) | 0.48 (**1.92×**) | 1.35 (**5.40×**) |
| anchored `^[a-z]+$` | 0.32 | unsupported | 0.42 (**1.31×**) | 2.18 (**6.81×**) |
| lookahead `[a-z]+(?=[a-z])` | 3.81 | 154.44 (**40.54×**) | 3.65 (0.96×) | unsupported |
...
**Stamp.** REAL `2026.8.7+` (tree `10aeafc`), both ISAs re-measured for this stamp, on the same
...
**3.411** (pcre2 **1.03×**, re2 **1.47×**), `\p{L}+` CJK **3.538** (0.81× / re2 **5.45×**), `\p{N}+`
**4.372** (0.59× / **1.71×**), `sc=Han` **5.548** (0.83×), `scx=Cyrl` **6.279** (**1.03×**), `(?i)café`
**0.669** (0.91× / **4.22×**), `[à-ÿ]+` **3.335** (**1.75×** / **7.63×**), literal `你好` **0.832**
(pcre2 **1.24×** — the same crossing as arm64), `.` emoji **8.267** (pcre2 **1.25×**), ascii witness
**1.751** (**3.67×**). Oracle: exhaustive `\p{L}` over U+0000..10FFFF (surrogates skipped) —
**0 mismatch**.
...
| --- | ---: | ---: | ---: | ---: | --- |
| `\w+` (mixed-script) | 1.946 | 60.55 (**31.12×**) | 1.86 (0.96×) | 3.20 (**1.64×**) | 16218/5406/16218/5406 ⚠ |
| `\w+` (mixed-script) | 1.948 | 59.62 (**30.61×**) | 1.86 (0.95×) | 3.19 (**1.64×**) | 16218/5406/16218/5406 ⚠ |
| `\p{L}+` (CJK) | 1.956 | unsupported | 1.33 (0.68×) | 13.23 (**6.76×**) | 12904/—/12904/12904 |
| `\p{N}+` (arabic digits) | 1.871 | unsupported | 1.62 (0.87×) | 5.22 (**2.79×**) | 6250/—/6250/6250 |
| `\p{sc=Han}` (CJK) | 2.272 | unsupported | 2.14 (0.94×) | unsupported | 25808/—/25808/— |
| `\p{scx=Cyrl}` (mixed-script) | 2.694 | unsupported | 2.69 (1.00×) | unsupported | 32436/—/32436/— |
| `(?i)café` (accented) | 0.404 | unsupported | 0.34 (0.84×) | 1.30 (**3.22×**) | 3509/—/3509/3509 |
| `[à-ÿ]+` (accented) | 1.574 | 86.60 (**55.02×**) | 2.05 (**1.30×**) | 12.53 (**7.96×**) | 38599/38599/38599/38599 |
| literal `你好` (CJK) | 0.457 | 28.84 (**63.11×**) | 0.58 (**1.27×**) | 2.62 (**5.73×**) | 6452/6452/6452/6452 |
| `.` (emoji, one codepoint) | 4.692 | 58.32 (**12.43×**) | 3.83 (0.82×) | 18.95 (**4.04×**) | 68306/200039/68306/68306 ⚠ |
| ascii witness `[a-z]+` | 1.164 | 90.80 (**78.01×**) | 2.25 (**1.93×**) | 13.87 (**11.92×**) | 42108/42108/42108/42108 |
