...
| | |
| --- | --- |
| Version | REAL `2026.7.59` — **re-measured for this stamp: §E.4 only** (the crate's own criterion rows, arm64, at `96f6d8b`), by an interleaved A/B against `v2026.7.58` with the same bench file on both sides, one group at a time, machine otherwise idle: three rounds for `first_use`, two for `captures`/`compile`, and **four rounds of 5 s for `find`** — not decoration, since `find/digits` read +12.5 % and `find/word_bound` +4.2 % on two rounds and **+0.8 %** and **+0.3 %** on four, per-round dispersion falling to 0.2 %. **The row this train was about:** `(\w+)@(\w+)` on first use had been disclosed and not chased in three consecutive releases — `first_use/email` **2.78 → 1.58 ms (−43.2 %)**, and 4.57× behind the `regex` crate becomes **2.66×**. Direct harness, construct plus one search, arm64: `(\w+)@(\w+)` 2729.3 → 1538.2 µs (**−43.6 %**), `(\d+)@(\d+)` 189.7 → 105.0 (**−44.6 %**); x86-64 instructions **−39.7 %**; allocations 601 486 → 236 566 blocks. Four steps, each picked by profiling per function and twice contradicting the obvious guess: the largest cost was `compute_lazy_alphabet` (26.5 %) and not the minimizer (19.1 %), whose byte-equivalence pass re-walked all 475 predicates per (byte, class) pair and now builds each byte's signature once; minimization rows split by what varies (41 words a round against 162) with the assigned targets packed; the trie's sequences moved from a vector OF vectors (164 220 blocks averaging 4.6 bytes) into one pool, and its recursion stopped re-allocating per interval per level; and the alphabet transpose reads each class's bitmap instead of asking `test` for all 256 bytes. **A compile-time defect the fuzzer found, not the bench suite:** the linear-time guarantee covers MATCHING, and nothing covered compile — `unicode_casefold` scanned all 2940 fold entries per class asking `any_of` over `\w`'s 771 ranges, once per repetition, so `\w{800}a` icase took **305 ms to COMPILE** and CI's fuzzer hit a 10-second timeout. Range-driven with a binary-search seek it is **51 ms (−83 %)**, and the reproducer 12.22 → 1.86 s. **What it costs:** nothing measurable — every other criterion row within ±1.7 %, the walk unchanged on the direct harness (`\w+` and `[a-z]+` 0.0 %, `\d+` −0.1 %), patterns whose build is already cheap unmoved. §A, §Unicode, §B and §multi-pattern carry their earlier figures unchanged. Per-train benchmark-impact log: CHANGELOG.md (full release notes: docs/release-notes/ + GitHub Releases). **Refuted and reverted, recorded so it is not retried:** callgrind put a `memset` in `minimize` at 4.87 % of the build; a round-stamp replacement moved arm64 wall clock −0.2 % and x86-64 instructions −0.2 % — `rep stos` is a known callgrind over-count, and no complexity was kept for it. |
