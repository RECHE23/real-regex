...
both ISAs, on the same `fe2fdb6` tree for this re-stamp.
...
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.72 | 29.03 (**16.90×**) | 6.43 (**3.74×**) | 28.28 (**16.46×**) |
| digits `[0-9]+` | 1.22 | 24.46 (**20.07×**) | 3.64 (**2.99×**) | 16.85 (**13.82×**) |
| fields `[^,]+` | 6.47 | 25.16 (**3.89×**) | 4.94 (0.76×) | 22.96 (**3.55×**) |
| alternation `the\|fox\|dog` | 2.35 | 29.65 (**12.64×**) | 2.25 (0.96×) | 10.45 (**4.46×**) |
| date `{4}-{2}-{2}` | 0.72 | 18.62 (**25.86×**) | 0.61 (0.85×) | 4.09 (**5.68×**) |
| hex `[0-9a-f]{8}` | 1.35 | 20.90 (**15.48×**) | 1.90 (**1.41×**) | 4.09 (**3.03×**) |
| literal | 0.43 | 14.99 (**34.62×**) | 0.59 (**1.36×**) | 2.40 (**5.54×**) |
| anchored `^[a-z]+$` | 0.52 | unsupported | 0.52 (**1.01×**) | 1.78 (**3.46×**) |
| lookahead `[a-z]+(?=[a-z])` | 7.59 | 76.52 (**10.09×**) | 6.80 (0.90×) | unsupported |

**arm64** — Apple M1 Pro, Apple clang 16, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 11.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.16 | 91.45 (**78.63×**) | 2.33 (**2.00×**) | 13.87 (**11.93×**) |
| digits `[0-9]+` | 0.85 | 82.07 (**97.01×**) | 1.44 (**1.70×**) | 8.42 (**9.95×**) |
| fields `[^,]+` | 3.23 | 74.88 (**23.16×**) | 1.87 (0.58×) | 11.04 (**3.41×**) |
| alternation `the\|fox\|dog` | 1.98 | 112.85 (**57.08×**) | 1.56 (0.79×) | 6.12 (**3.10×**) |
| date `{4}-{2}-{2}` | 0.49 | 71.47 (**146.76×**) | 0.39 (0.80×) | 3.41 (**7.00×**) |
| hex `[0-9a-f]{8}` | 1.42 | 80.30 (**56.75×**) | 1.24 (0.88×) | 3.42 (**2.42×**) |
| literal | 0.25 | 30.61 (**123.93×**) | 0.48 (**1.94×**) | 1.35 (**5.47×**) |
| anchored `^[a-z]+$` | 0.32 | unsupported | 0.42 (**1.32×**) | 2.18 (**6.83×**) |
| lookahead `[a-z]+(?=[a-z])` | 4.00 | 155.20 (**38.80×**) | 3.66 (0.92×) | unsupported |
...
**Stamp.** REAL `2026.8.5+` (tree `fe2fdb6`), both ISAs re-measured for this stamp, on the same
...
**3.679** (pcre2 0.97×, re2 **1.36×**), `\p{L}+` CJK **4.195** (0.70× / re2 **4.60×**), `\p{N}+`
**4.390** (0.59× / **1.70×**), `sc=Han` **6.049** (0.76×), `scx=Cyrl` **6.407** (**1.02×**), `(?i)café`
**0.671** (**0.89×** / **4.16×**), `[à-ÿ]+` **3.368** (**1.74×** / **7.51×**), literal `你好` **0.821**
(pcre2 **1.25×** — the same crossing as arm64), `.` emoji **8.685** (pcre2 **1.19×**), ascii witness
**1.719** (**3.73×**). Oracle: exhaustive `\p{L}` over U+0000..10FFFF (surrogates skipped) —
**0 mismatch**.
...
| --- | ---: | ---: | ---: | ---: | --- |
| `\w+` (mixed-script) | 2.295 | 60.33 (**26.29×**) | 1.86 (0.81×) | 3.20 (**1.39×**) | 16218/5406/16218/5406 ⚠ |
| `\p{L}+` (CJK) | 2.852 | unsupported | 1.33 (0.47×) | 13.13 (**4.60×**) | 12904/—/12904/12904 |
| `\p{N}+` (arabic digits) | 1.872 | unsupported | 1.61 (0.86×) | 5.20 (**2.78×**) | 6250/—/6250/6250 |
| `\p{sc=Han}` (CJK) | 3.328 | unsupported | 2.14 (0.64×) | unsupported | 25808/—/25808/— |
| `\p{scx=Cyrl}` (mixed-script) | 2.654 | unsupported | 2.67 (**1.01×**) | unsupported | 32436/—/32436/— |
| `(?i)café` (accented) | 0.404 | unsupported | 0.34 (0.84×) | 1.30 (**3.22×**) | 3509/—/3509/3509 |
| `[à-ÿ]+` (accented) | 1.572 | 86.45 (**54.99×**) | 2.05 (**1.30×**) | 12.51 (**7.96×**) | 38599/38599/38599/38599 |
| literal `你好` (CJK) | 0.457 | 29.04 (**63.54×**) | 0.58 (**1.27×**) | 2.62 (**5.73×**) | 6452/6452/6452/6452 |
| `.` (emoji, one codepoint) | 4.668 | 58.81 (**12.60×**) | 3.73 (0.80×) | 18.76 (**4.02×**) | 68306/200039/68306/68306 ⚠ |
| ascii witness `[a-z]+` | 1.163 | 91.99 (**79.10×**) | 2.25 (**1.93×**) | 13.86 (**11.92×**) | 42108/42108/42108/42108 |
