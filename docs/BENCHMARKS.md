...
both ISAs, on the same `366c002` tree for this re-stamp.

**This stamp moves on both ISAs, and almost none of it is engine code.** The train behind it lifts a
capability (possessive quantifiers over non-ASCII literals) and takes a route that was missing
(`é+` 5.62 → 1.765 ns/B, `[éàèùç]+` 8.44 → 1.915), neither of which has a row here. What the rows do
show is the translation unit moving under them: on arm64 the median is +0.02 % with `[à-ÿ]+` at
**+5.15 %**; on x86-64 the median is −0.50 % with `digits` at **−10.36 %**, `words` −5.72 % and its
ASCII witness −3.99 % against `date` +3.82 %.

**`digits` swinging back is the tell.** The previous stamp published it +12 % WORSE for the same
reason in the other direction, and this document said then that the mechanism was code placement.
Here the discriminator is applied rather than assumed: `[à-ÿ]+`, the largest arm64 mover, compiles to
a **byte-identical program** across the two tags — 8 instructions, one `klass_cp`, no `byte` — checked
against a worktree at `v2026.8.9`'s predecessor rather than inferred. Its 5.15 % is therefore not
engine code. The rows this train genuinely changed are the accented ones with no table row, and
`[éàèùç]+`, whose program did change (6 instructions with a `byte` → 8 with a `klass_cp`).
...
machine, and this set says so again: arm64 `words` reads 1.184 / 1.237 / 1.164 — 6.3 % — while
`alt` and `literal` hold inside 0.2 % across the same three runs; x86-64 is tighter but not clean
either (`fields` 2.5 %, `literal` 2.2 %, `anchored` 0.3 %). That is the episode shape described under
Methodology: a burst that lands on whatever is scanning when it arrives, not a property of the row. A
minimum across runs is what survives it; a mean would not. **33 of the published REAL cells came from
a run other than the first** — 16 on arm64, 17 on x86-64 — which is the protocol earning its cost
...
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.66 | 29.03 (**17.49×**) | 6.44 (**3.88×**) | 28.23 (**17.01×**) |
| digits `[0-9]+` | 1.08 | 24.38 (**22.57×**) | 3.64 (**3.37×**) | 16.83 (**15.58×**) |
| fields `[^,]+` | 6.15 | 25.21 (**4.10×**) | 4.95 (0.80×) | 23.03 (**3.74×**) |
| alternation `the\|fox\|dog` | 2.24 | 29.60 (**13.21×**) | 2.26 (**1.01×**) | 10.42 (**4.65×**) |
| date `{4}-{2}-{2}` | 0.72 | 18.48 (**25.67×**) | 0.61 (0.85×) | 4.09 (**5.68×**) |
| hex `[0-9a-f]{8}` | 1.36 | 20.89 (**15.36×**) | 1.89 (**1.39×**) | 4.09 (**3.01×**) |
| literal | 0.43 | 14.99 (**34.86×**) | 0.59 (**1.37×**) | 2.38 (**5.53×**) |
| anchored `^[a-z]+$` | 0.51 | unsupported | 0.52 (**1.02×**) | 1.78 (**3.49×**) |
| lookahead `[a-z]+(?=[a-z])` | 7.57 | 76.16 (**10.06×**) | 6.81 (0.90×) | unsupported |

**arm64** — Apple M1 Pro, Apple clang 16, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 11.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.16 | 91.99 (**79.30×**) | 2.32 (**2.00×**) | 13.87 (**11.96×**) |
| digits `[0-9]+` | 0.85 | 81.93 (**96.39×**) | 1.44 (**1.69×**) | 8.62 (**10.14×**) |
| fields `[^,]+` | 3.21 | 73.59 (**22.93×**) | 1.87 (0.58×) | 11.05 (**3.44×**) |
| alternation `the\|fox\|dog` | 1.95 | 113.30 (**58.10×**) | 1.57 (0.81×) | 6.08 (**3.12×**) |
| date `{4}-{2}-{2}` | 0.49 | 72.09 (**147.12×**) | 0.39 (0.80×) | 3.41 (**6.96×**) |
| hex `[0-9a-f]{8}` | 1.42 | 80.13 (**56.43×**) | 1.24 (0.87×) | 3.40 (**2.39×**) |
| literal | 0.25 | 31.02 (**124.08×**) | 0.48 (**1.92×**) | 1.35 (**5.40×**) |
| anchored `^[a-z]+$` | 0.32 | unsupported | 0.42 (**1.31×**) | 2.18 (**6.81×**) |
| lookahead `[a-z]+(?=[a-z])` | 3.81 | 155.29 (**40.76×**) | 3.65 (0.96×) | unsupported |
...
**Stamp.** REAL `2026.8.8+` (tree `366c002`), both ISAs re-measured for this stamp, on the same
...
**3.448** (pcre2 **1.01×**, re2 **1.45×**), `\p{L}+` CJK **3.402** (0.83× / re2 **5.65×**), `\p{N}+`
**4.232** (0.61× / **1.77×**), `sc=Han` **5.680** (0.81×), `scx=Cyrl` **6.347** (**1.01×**), `(?i)café`
**0.682** (0.88× / **4.34×**), `[à-ÿ]+` **3.424** (**1.72×** / **7.37×**), literal `你好` **0.838**
(pcre2 **1.23×** — the same crossing as arm64), `.` emoji **7.789** (pcre2 **1.33×**), ascii witness
**1.681** (**3.82×**). Oracle: exhaustive `\p{L}` over U+0000..10FFFF (surrogates skipped) —
**0 mismatch**.
...
| `\w+` (mixed-script) | 1.942 | 60.53 (**31.17×**) | 1.86 (0.96×) | 3.19 (**1.64×**) | 16218/5406/16218/5406 ⚠ |
| `\p{L}+` (CJK) | 1.961 | unsupported | 1.33 (0.68×) | 13.21 (**6.74×**) | 12904/—/12904/12904 |
| `\p{N}+` (arabic digits) | 1.871 | unsupported | 1.61 (0.86×) | 5.21 (**2.78×**) | 6250/—/6250/6250 |
| `\p{sc=Han}` (CJK) | 2.273 | unsupported | 2.15 (0.95×) | unsupported | 25808/—/25808/— |
| `\p{scx=Cyrl}` (mixed-script) | 2.699 | unsupported | 2.70 (**1.00×**) | unsupported | 32436/—/32436/— |
| `(?i)café` (accented) | 0.404 | unsupported | 0.34 (0.84×) | 1.31 (**3.24×**) | 3509/—/3509/3509 |
| `[à-ÿ]+` (accented) | 1.655 | 87.66 (**52.97×**) | 2.06 (**1.24×**) | 12.55 (**7.58×**) | 38599/38599/38599/38599 |
| literal `你好` (CJK) | 0.457 | 29.02 (**63.50×**) | 0.58 (**1.27×**) | 2.62 (**5.73×**) | 6452/6452/6452/6452 |
| `.` (emoji, one codepoint) | 4.691 | 58.40 (**12.45×**) | 3.81 (0.81×) | 18.93 (**4.04×**) | 68306/200039/68306/68306 ⚠ |
| ascii witness `[a-z]+` | 1.164 | 91.03 (**78.20×**) | 2.25 (**1.93×**) | 13.95 (**11.98×**) | 42108/42108/42108/42108 |
