...
both ISAs, on the same `e41ac70` tree for this re-stamp.
...
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.69 | 29.34 (**17.39×**) | 6.42 (**3.81×**) | 28.12 (**16.67×**) |
| digits `[0-9]+` | 1.18 | 24.24 (**20.51×**) | 3.64 (**3.08×**) | 16.71 (**14.14×**) |
| fields `[^,]+` | 5.93 | 25.04 (**4.22×**) | 4.96 (0.84×) | 22.95 (**3.87×**) |
| alternation `the\|fox\|dog` | 2.26 | 30.12 (**13.34×**) | 2.26 (**1.00×**) | 10.37 (**4.59×**) |
| date `{4}-{2}-{2}` | 0.68 | 17.98 (**26.36×**) | 0.61 (0.89×) | 4.19 (**6.14×**) |
| hex `[0-9a-f]{8}` | 1.36 | 20.37 (**15.00×**) | 1.91 (**1.41×**) | 4.09 (**3.01×**) |
| literal | 0.43 | 15.01 (**34.59×**) | 0.59 (**1.36×**) | 2.38 (**5.48×**) |
| anchored `^[a-z]+$` | 0.52 | unsupported | 0.52 (**1.01×**) | 1.78 (**3.46×**) |
| lookahead `[a-z]+(?=[a-z])` | 7.78 | 76.15 (**9.79×**) | 6.79 (0.87×) | unsupported |

**arm64** — Apple M1 Pro, Apple clang 16, N = 30 × 3 runs, PCRE2 10.47-JIT, RE2 11.0:

| case | REAL ns/B | std::regex | PCRE2-JIT | RE2 |
| --- | ---: | ---: | ---: | ---: |
| words `[a-z]+` | 1.20 | 92.63 (**77.51×**) | 2.32 (**1.94×**) | 13.88 (**11.62×**) |
| digits `[0-9]+` | 0.84 | 82.25 (**97.34×**) | 1.44 (**1.70×**) | 8.78 (**10.39×**) |
| fields `[^,]+` | 3.20 | 75.83 (**23.70×**) | 1.87 (0.58×) | 11.03 (**3.45×**) |
| alternation `the\|fox\|dog` | 1.98 | 114.44 (**57.68×**) | 1.56 (0.79×) | 6.13 (**3.09×**) |
| date `{4}-{2}-{2}` | 0.49 | 71.62 (**146.16×**) | 0.39 (0.80×) | 3.42 (**6.98×**) |
| hex `[0-9a-f]{8}` | 1.42 | 79.76 (**56.33×**) | 1.24 (0.88×) | 3.41 (**2.41×**) |
| literal | 0.25 | 31.00 (**125.00×**) | 0.48 (**1.94×**) | 1.36 (**5.48×**) |
| anchored `^[a-z]+$` | 0.32 | unsupported | 0.42 (**1.32×**) | 2.18 (**6.83×**) |
| lookahead `[a-z]+(?=[a-z])` | 4.01 | 156.41 (**39.02×**) | 3.65 (0.91×) | unsupported |
...
**Stamp.** REAL `2026.8.6+` (tree `e41ac70`), both ISAs re-measured for this stamp, on the same
...
**3.324** (pcre2 **1.05×**, re2 **1.49×**), `\p{L}+` CJK **3.475** (0.81× / re2 **5.53×**), `\p{N}+`
**4.215** (0.61× / **1.77×**), `sc=Han` **5.398** (0.85×), `scx=Cyrl` **6.343** (**1.01×**), `(?i)café`
**0.655** (0.92× / **4.29×**), `[à-ÿ]+` **3.282** (**1.79×** / **7.69×**), literal `你好` **0.828**
(pcre2 **1.24×** — the same crossing as arm64), `.` emoji **7.586** (pcre2 **1.37×**), ascii witness
**1.684** (**3.81×**). Oracle: exhaustive `\p{L}` over U+0000..10FFFF (surrogates skipped) —
**0 mismatch**.
...
| --- | ---: | ---: | ---: | ---: | --- |
| `\w+` (mixed-script) | 1.945 | 60.40 (**31.05×**) | 1.86 (0.96×) | 3.19 (**1.64×**) | 16218/5406/16218/5406 ⚠ |
| `\p{L}+` (CJK) | 1.949 | unsupported | 1.33 (0.68×) | 13.19 (**6.77×**) | 12904/—/12904/12904 |
| `\p{N}+` (arabic digits) | 1.873 | unsupported | 1.59 (0.85×) | 5.19 (**2.77×**) | 6250/—/6250/6250 |
| `\p{sc=Han}` (CJK) | 2.271 | unsupported | 2.14 (0.94×) | unsupported | 25808/—/25808/— |
| `\p{scx=Cyrl}` (mixed-script) | 2.690 | unsupported | 2.67 (0.99×) | unsupported | 32436/—/32436/— |
| `(?i)café` (accented) | 0.404 | unsupported | 0.34 (0.84×) | 1.31 (**3.24×**) | 3509/—/3509/3509 |
| `[à-ÿ]+` (accented) | 1.574 | 86.81 (**55.15×**) | 2.05 (**1.30×**) | 12.52 (**7.95×**) | 38599/38599/38599/38599 |
| literal `你好` (CJK) | 0.457 | 28.85 (**63.13×**) | 0.58 (**1.27×**) | 2.62 (**5.73×**) | 6452/6452/6452/6452 |
| `.` (emoji, one codepoint) | 4.768 | 58.81 (**12.33×**) | 3.71 (0.78×) | 18.94 (**3.97×**) | 68306/200039/68306/68306 ⚠ |
| ascii witness `[a-z]+` | 1.164 | 91.75 (**78.82×**) | 2.25 (**1.93×**) | 14.07 (**12.09×**) | 42108/42108/42108/42108 |
