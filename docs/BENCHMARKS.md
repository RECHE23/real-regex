...
| | |
| --- | --- |
| Version | REAL `2026.7.35` — §A/§E re-stamped on `a994ff9` (post O2r-1b, the A2 fix, the P0 #2 icase fix + seed-128 tune, and the mono/multi cascade split), 2026-07-11; §Unicode's `.`/ascii-witness cells re-stamped on `258783b` (post wagon-4 strict-UTF-8 fix for `.`), 2026-07-11 — every section carrying a re-stamp note is now re-stamped within this same train, not a stale carry-over. 7.35 (D1, possessive quantifiers / atomic groups) adds no case to this suite and touches no EXISTING measured code path — confirmed flat by its own interleaved-binaries A/B, not a re-stamp trigger; the possessive-vs-greedy cost itself is a separate, disclosed finding (see the v2026.7.35 release notes), not part of this baseline. 7.34 (the P1 `\b`/`\B`-junction fix) touches no measured code path — confirmed flat by its own dual-ISA A/B, not a re-stamp trigger. §multi-pattern unchanged (Stage-2 + Arc I/II + Arc B `\b` unlock) |
